...
sed -i 's|export KEY_CITY="SanFrancisco"|export KEY_CITY="Bekasi"|' /etc/openvpn/easy-rsa/2.0/vars
sed -i 's|export KEY_ORG="Fort-Funston"|export KEY_ORG="Dunia-internet.xyz"|' /etc/openvpn/easy-rsa/2.0/vars
sed -i 's|export KEY_EMAIL="me@myhost.mydomain"|export KEY_EMAIL="sales@hostingtermurah.net"|' /etc/openvpn/easy-rsa/2.0/vars
sed -i 's|export KEY_EMAIL=mail@host.domain|export KEY_EMAIL=admin@Dunia-internet.xyz|' /etc/openvpn/easy-rsa/2.0/vars
sed -i 's|export KEY_CN=changeme|export KEY_CN="Dunia-internet.xyz"|' /etc/openvpn/easy-rsa/2.0/vars
sed -i 's|export KEY_NAME=changeme|export KEY_NAME=Dunia-internet.xyz|' /etc/openvpn/easy-rsa/2.0/vars
sed -i 's|export KEY_OU=changeme|export KEY_OU=Dunia-internet.xyz|' /etc/openvpn/easy-rsa/2.0/vars
# Buat PKI
...
sed -i 's/DROPBEAR_EXTRA_ARGS=/DROPBEAR_EXTRA_ARGS="-p 109 -p 3128"/g' /etc/default/dropbear
...
echo "                         Copyright DUNIA-INTERNET.xyz                         "
